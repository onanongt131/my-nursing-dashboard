...
export default function CommitteeDetailPage() {
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center min-h-[400px]">
        <p className="text-emerald-800 font-medium">กำลังโหลดข้อมูล...</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full space-y-6 pb-12">
      {/* ส่วนหัวชื่อคณะกรรมการ และปุ่มแก้ไข/เพิ่มข้อมูล */}
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between border-b-2 border-amber-400 pb-4 gap-4">
        <h1 className="text-xl sm:text-2xl font-bold text-emerald-900">
          {committeeData?.committee_name || "คณะกรรมการ"}
        </h1>
        
        <button
          type="button"
          onClick={() => router.push(`/dashboard/committee/${committeeKey}/edit`)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        {/* คอลัมน์ที่ 1: ภาพประธานคณะกรรมการ และเอกสารดาวน์โหลด */}
        <div className="lg:col-span-3 space-y-6">
          <div className="bg-white rounded-2xl border border-emerald-200 shadow-sm overflow-hidden">
            ...
            <div className="p-4 flex flex-col items-center">
              {/* ขยายความกว้างรูปภาพให้เกือบชิดขอบการ์ด (w-full พร้อมกำหนด min-h และ object-cover) */}
              <div className="w-full h-64 bg-gray-100 rounded-xl overflow-hidden border-2 border-amber-300 shadow-inner mb-3 flex items-center justify-center">
                {committeeData?.president_image ? (
                  <img 
                    src={committeeData.president_image} 
                    alt="ประธานคณะกรรมการ" 
              ...
              <p className="font-bold text-gray-800 text-center text-sm">
                {committeeData?.president_name || "รอยืนยันรายนาม"}
              </p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-emerald-200 shadow-sm overflow-hidden">
            ...
            <div className="p-3 space-y-2">
              {committeeData?.documents && committeeData.documents.length > 0 ? (
                committeeData.documents.map((doc: any, index: number) => (
                  <a
                    key={index}
                    href={doc.url}
                    target="_blank"
        ...
        {/* คอลัมน์ที่ 2: บทบาทหน้าที่ และรายชื่อคณะกรรมการ */}
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-white rounded-2xl border border-emerald-200 shadow-sm overflow-hidden">
            ...
            <div className="p-5 text-gray-700 text-sm leading-relaxed min-h-[160px]">
              {committeeData?.roles_image ? (
                <div className="flex justify-center">
                  <img src={committeeData.roles_image} alt="บทบาทหน้าที่" className="max-h-96 object-contain rounded-lg border" />
                </div>
              ) : committeeData?.roles_and_duties ? (
                <div className="whitespace-pre-line">{committeeData.roles_and_duties}</div>
              ) : (
          ...
          <div className="bg-white rounded-2xl border border-emerald-200 shadow-sm overflow-hidden">
            ...
            <div className="p-5">
              {committeeData?.members_image ? (
                <div className="flex justify-center">
                  <img src={committeeData.members_image} alt="รายชื่อคณะกรรมการ" className="max-h-96 object-contain rounded-lg border" />
                </div>
              ) : committeeData?.committee_members && committeeData.committee_members.length > 0 ? (
                <ul className="space-y-2 text-sm text-gray-700">
                  {committeeData.committee_members.map((member: any, index: number) => (
        ...
        {/* คอลัมน์ที่ 3: ภาพกิจกรรม (แสดงผลแบบคู่ 2 คอลัมน์ เต็มสัดส่วนแนวตั้ง + คลิกดูภาพใหญ่) */}
<div className="lg:col-span-4 space-y-6">
  <div className="bg-white rounded-2xl border border-emerald-200 shadow-sm overflow-hidden">
    ...
    <div className="p-5">
      {committeeData?.activity_images && committeeData.activity_images.length > 0 ? (
        <div className="grid grid-cols-2 gap-4">
          {committeeData.activity_images.map((imgItem: any, index: number) => {
...
{selectedImage && (
  ...
  >
    <div className="relative max-w-4xl w-full max-h-[90vh] flex items-center justify-center">
      <button 
        onClick={() => setSelectedImage(null)}
        className="absolute -top-10 right-0 text-white hover:text-amber-300 transition-colors p-2"
      >
