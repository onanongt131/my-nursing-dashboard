...
export default function DepartmentPage() {
  ...
  // ฟังก์ชันดึงข้อมูลรายการรออนุมัติจากทุกตารางระบบ
  ...
  return (
    <div className="px-2 py-2 max-w-full mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 items-start">
        {/* Sidebar เลือกกลุ่มงานและหน่วยงาน */}
              ...
              <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-6">
                <div className="border-b pb-4 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {/* ปุ่มแจ้งเตือนรออนุมัติสีแดงเด่นชัด */}
                    <button 
                      onClick={() => setShowPendingTable(!showPendingTable)} 
                      className={`px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 shadow-sm transition-all cursor-pointer ${
                        totalPendingCount > 0 
                    ...
                    <button onClick={() => setActiveModal('readmit')} className="bg-teal-700 hover:bg-teal-800 text-white px-3 py-2 rounded-xl text-sm font-bold shadow transition flex items-center gap-1.5">
                      <span>Re-admit</span>
                    </button>
                    <button onClick={() => setActiveModal('ama')} className="bg-amber-600 hover:bg-amber-700 text-white px-4 py-2 rounded-xl text-sm font-bold shadow transition flex items-center gap-1.5">
